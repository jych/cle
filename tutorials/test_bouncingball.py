...
def inner_fn(s1_tm1, s2_tm1, s3_tm1, iter_):
    ...
    s2_t = h2.fprop([[iter_, s1_t], s2_tm1])
    s3_t = h3.fprop([[iter_, s2_t], s3_tm1])
...
test_fn = theano.function(inputs=[x, N_steps],
                          outputs=[y_hat_temp],
                          updates=updates,
...
dummy_n_steps = 100
samples = test_fn(seed, dummy_n_steps)[-1]
ipdb.set_trace()
